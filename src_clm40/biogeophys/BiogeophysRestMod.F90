...
!
    ...
    real(r8), pointer :: ptr1d(:), ptr2d(:,:)     ! does this overcome each tracer array 
                                                  ! not inheriting pointer status?
    real(r8) :: rtmp_liq(pwtrc), rtmp_ice(pwtrc)  ! temp isotope ratios
!-----------------------------------------------------------------------
    filetypes(:)           = "missing"
    ...
    !----------------------
    if(hydro_tracer) then !Are water tracers on?
      do m=1,pwtrc !loop over water tracers
        if (flag == 'define') then 
           call ncd_defvar(ncid=ncid, varname='WA_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='column', &
                long_name=trim(wtrcnam(m))//' water in the unconfined aquifer', units='mm')
        else if (flag == 'read' .or. flag == 'write') then 
           ptr1d => cws%wtr_wa(:,m) !point to data
           call ncd_io(varname='WA_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namec, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then 
              if (is_restart()) call endrun()
              ...
              if (.not. is_restart()) then 
                write(6,*) 'ERROR: reading restart file for tracer-CLM initialization, &
                            but some/all tracers missing on file. Ending model now.'
                call endrun()
              end if
           end if
        end if
   ...
   !-------------
    ...
    if(hydro_tracer) then !Are water tracers on?
      do m=1,pwtrc !loop over water tracers
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='WT_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='column', &
                long_name=trim(wtrcnam(m))//' total water storage', units='mm')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => cws%wtr_wt(:,m) !point to data
           call ncd_io(varname='WT_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namec, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
   ...
   !-------------
    ...
    if(hydro_tracer) then !Are water tracers on?
      do m=1,pwtrc !loop over water tracers
        if (flag == 'define') then 
           call ncd_defvar(ncid=ncid, varname='RGNIP_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='column', &
                long_name=trim(wtrcnam(m))//' GNIP isotope ratio', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then 
           ptr1d => cws%RGNIP(:,m) !point to data
           call ncd_io(varname='RGNIP_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namec, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then 
              if (is_restart()) call endrun()
...
!!DEBUG - in the past, need to use the "single_column_hack" feature to get the restart files to be read correctly -TW
!! (modify: /models/lnd/clm/src/util_share/ncdio_pio.F90)
            dim1name=namep, switchdim=.true., ncid=ncid, flag=flag, readvar=readvar)
            !!dim1name=namep, switchdim=.true., ncid=ncid, flag=flag, readvar=readvar, &
            !!single_column_hack=.true.) !TW
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) call endrun()
       end if
    end if
...
!DEBUG
   ...
   ! column water state variable - h2osno
    ...
    if(hydro_tracer) then !Are water tracers on?
      do m=1,pwtrc !loop over water tracers
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='H2OSNO_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='column', &
                long_name=trim(wtrcnam(m))//' snow water', units='kg/m2')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => cws%wtr_h2osno(:,m) !point to data
           call ncd_io(varname='H2OSNO_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namec, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
    ...
    if(hydro_tracer) then !Are water tracers on?
      do m=1,pwtrc !loop over water tracers
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='H2OSOI_LIQ_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='column', dim2name='levtot', switchdim=.true., &
                long_name=trim(wtrcnam(m))//' liquid water', units='kg/m2')
        else if (flag == 'read' .or. flag == 'write') then
           ptr2d => cws%wtr_h2osoi_liq(:,:,m)
           call ncd_io(varname='H2OSOI_LIQ_'//trim(wtrcnam(m)),data=ptr2d, &
                dim1name=namec, switchdim=.true., ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
              ptr2d(:,:) = 0.0_r8
    ...
    if(hydro_tracer) then !Are water tracers on?
      do m=1,pwtrc !loop over water tracers
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='H2OSOI_ICE_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='column', dim2name='levtot', switchdim=.true., &
                long_name=trim(wtrcnam(m))//' ice lens', units='kg/m2')
        else if (flag == 'read' .or. flag == 'write') then
           ptr2d => cws%wtr_h2osoi_ice(:,:,m)
           call ncd_io(varname='H2OSOI_ICE_'//trim(wtrcnam(m)), data=ptr2d, &
                dim1name=namec, switchdim=.true., ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
              ptr2d(:,:) = 0.0_r8
...
!DEBUG
    ...
    !Water tracers/isotopes
    !----------------------
    if(hydro_tracer) then

      do m = 1, pwtrc

        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='H2OCAN_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' canopy water', units='kg/m2')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%wtr_h2ocan(:,m)
           call ncd_io(varname='H2OCAN_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rcanopy_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' canopy vapor ratio', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RCanopyVapor(:,m) !point to data
           call ncd_io(varname='Rcanopy_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then 
           call ncd_defvar(ncid=ncid, varname='Rcanopywater_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' canopy water ratio', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then 
           ptr1d => pws%RCanopyWater(:,m) !point to data
           call ncd_io(varname='Rcanopywater_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then 
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rleafsun_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' sun leaf water ratio', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RLeafWaterSun(:,m) !point to data
           call ncd_io(varname='Rleafsun_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rleafsha_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' shade leaf water ratio', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RLeafWaterSha(:,m) !point to data
           call ncd_io(varname='Rleafsha_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        ! Cellulose isotope ratio (sunlit/shaded)
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rcellsun_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit cellulose (incl Peclet)', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RCelluloseSun(:,m) !point to data
           call ncd_io(varname='Rcellsun_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rcellsha_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of shaded cellulose (incl Peclet)', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RCelluloseSha(:,m) !point to data
           call ncd_io(varname='Rcellsha_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        ! Cellulose isotope ratio times photosynthesis (sunlit/shaded)
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rcellsunpsn_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit cellulose (incl Peclet), times sunlit photosynthesis', units='umolCO2/m^2/s')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RCelluloseSunPsn(:,m) !point to data
           call ncd_io(varname='Rcellsunpsn_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rcellshapsn_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of shaded cellulose (incl Peclet), times sunlit photosynthesis', units='umolCO2/m^2/s')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RCelluloseShaPsn(:,m) !point to data
           call ncd_io(varname='Rcellshapsn_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        ! Diagnostic (Peclet effect included) leaf water ratios (sunlit/shaded)
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rleafsundiag_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit leaf water (incl Peclet)', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RLeafWaterSunDiag(:,m) !point to data
           call ncd_io(varname='Rleafsundiag_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rleafshadiag_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of shaded leaf water (incl Peclet)', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RLeafWaterShaDiag(:,m) !point to data
           call ncd_io(varname='Rleafshadiag_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        ! Steady state leaf water ratios (sunlit/shaded)
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rleafsunstdy_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit leaf water (steady state)', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RLeafWaterSunStdy(:,m) !point to data
           call ncd_io(varname='Rleafsunstdy_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rleafshastdy_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of shaded leaf water (steady state)', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RLeafWaterShaStdy(:,m) !point to data
           call ncd_io(varname='Rleafshastdy_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        ! Leaf water ratios times photosynthesis (sunlit/shaded)
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rleafsunpsn_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit leaf water (incl Peclet), times sunlit photosynthesis', units='umolCO2/m^2/s')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RLeafWaterSunPsn(:,m) !point to data
           call ncd_io(varname='Rleafsunpsn_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rleafshapsn_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of shaded leaf water (incl Peclet), times shaded photosynthesis', units='umolCO2/m^2/s')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RLeafWaterShaPsn(:,m) !point to data
           call ncd_io(varname='Rleafshapsn_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
        ...
        ! Xylem water ratios (root-weighted soil)
        if (flag == 'define') then
           call ncd_defvar(ncid=ncid, varname='Rxylem_'//trim(wtrcnam(m)), xtype=ncd_double,  &
                dim1name='pft', &
                long_name=trim(wtrcnam(m))//' to H2O ratio of xylem water', units='unitless')
        else if (flag == 'read' .or. flag == 'write') then
           ptr1d => pws%RXylem(:,m) !point to data
           call ncd_io(varname='Rxylem_'//trim(wtrcnam(m)), data=ptr1d, &
                dim1name=namep, &
                ncid=ncid, flag=flag, readvar=readvar)
           if (flag=='read' .and. .not. readvar) then
              if (is_restart()) call endrun()
          ...
          do c = begc,endc
             ...
             do j = 1,nlevs
                ...
                if (ltype(l) == istsoil .or. ltype(l) == istcrop) then
                   ...
                   cws%h2osoi_vol(c,j) = cws%h2osoi_liq(c,j)/(cps%dz(c,j)*denh2o) &
                                              + cws%h2osoi_ice(c,j)/(cps%dz(c,j)*denice)
                   ! Needed for isotopic initialization - TW (15 Aug 2014)
                   do m = 1, pwtrc
                      cws%wtr_h2osoi_liq(c,j,m) = max(0._r8,cws%wtr_h2osoi_liq(c,j,m))
                      cws%wtr_h2osoi_ice(c,j,m) = max(0._r8,cws%wtr_h2osoi_ice(c,j,m))
                      cws%wtr_h2osoi_vol(c,j,m) = cws%wtr_h2osoi_liq(c,j,m)/(cps%dz(c,j)*denh2o) &
                                                 + cws%wtr_h2osoi_ice(c,j,m)/(cps%dz(c,j)*denice)
                   end do
                   if (j == 1) then
                   ...
                   if (cws%h2osoi_vol(c,j) > maxwatsat) then 
                      ...
                      ! Needed for isotopic initialization - TW (15 Aug 2014)
                      do m = 1, pwtrc
                         cws%wtr_h2osoi_liq(c,j,m) = cws%wtr_h2osoi_liq(c,j,m) - &
                           (cws%wtr_h2osoi_liq(c,j,m)/totwat) * excess
                         cws%wtr_h2osoi_ice(c,j,m) = cws%wtr_h2osoi_ice(c,j,m) - &
                           (cws%wtr_h2osoi_ice(c,j,m)/totwat) * excess
                      end do
                   end if
                   ! Needed for isotopic initialization - TW (15 Aug 2014)
                   ! Preserve the ratio if applying a watmin - 
                   ! already made sure these are positive, so okay there
                   ! Leave this here for now, but use RSMOW instead for these fixes
                   do m = 1, pwtrc
                      rtmp_liq(m) = get_wratio( cws%wtr_h2osoi_liq(c,j,m), cws%wtr_h2osoi_liq(c,m,ixbase), h2otiny)
                      rtmp_ice(m) = get_wratio( cws%wtr_h2osoi_ice(c,j,m), cws%wtr_h2osoi_ice(c,m,ixbase), h2otiny)
                   end do
                   cws%h2osoi_liq(c,j) = max(watmin,cws%h2osoi_liq(c,j))
                   cws%h2osoi_ice(c,j) = max(watmin,cws%h2osoi_ice(c,j))
                   cws%h2osoi_vol(c,j) = cws%h2osoi_liq(c,j)/(cps%dz(c,j)*denh2o) &
                                              + cws%h2osoi_ice(c,j)/(cps%dz(c,j)*denice)
                   if (cws%h2osoi_liq(c,j) == watmin) then
                      do m = 1, pwtrc
                         !!cws%wtr_h2osoi_liq(c,j,m) = rtmp_liq(m)*watmin
                         cws%wtr_h2osoi_liq(c,j,m) = Rstnd(m)*watmin
                      end do
                   end if
                   if (cws%h2osoi_ice(c,j) == watmin) then
                      do m = 1, pwtrc
                         !!cws%wtr_h2osoi_ice(c,j,m) = rtmp_ice(m)*watmin
                         cws%wtr_h2osoi_ice(c,j,m) = Rstnd(m)*watmin
                      end do
                   end if
                   do m = 1, pwtrc
                      cws%wtr_h2osoi_vol(c,j,m) = cws%wtr_h2osoi_liq(c,j,m)/(cps%dz(c,j)*denh2o) &
       ...
       if (flag == 'read' .and. .not. readvar) then
          if (is_restart()) then
             ...
             do c=begc,endc
                ...
                do m = 1, pwtrc
                   cwf%wtr_qflx_snofrz_lyr(c,-nlevsno+1:0,m) = 0._r8
                end do
             enddo
          endif
       end if
