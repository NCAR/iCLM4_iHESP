...
!!               - qflx_qrgwl(c) - qflx_drain(c) - qflx_snwcp_ice(c)) * dtime
      ...
      found = .false.
      do c = lbc, ubc
         if (cwtgcell(c) > 0._r8 .and. abs(wtr_errh2o(c,m)) > 10._r8*Rstnd(m)) then ! 1e-2 for production; 1e-5 for debugging
            found = .true.
            indexc = c
            exit
         end if
      ...
      if ( found .and. (nstep > 2) ) then
         write(iulog,*)'WARNING: tracer water balance error',&
                        ...
                        ' landunit type= ',ltype(clandunit(indexc)),' c=',indexc,' p=',col%pfti(indexc),col%pftf(indexc)
         write(6,*)'   error ratio to Rsnd (', Rstnd(m), ') = ', &
                    abs(wtr_errh2o(indexc,m))/Rstnd(m)

        if(cwtgcell(c) > 0._r8 .and. abs(wtr_errh2o(c,m)) > 10.*Rstnd(m)) then !model is REALLY screwing up - kill it
