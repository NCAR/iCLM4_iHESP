...
! !USES:
    ...
    use HydrologyTracer, only : pwtrc, wtrcnam, pdbg, cdbg, mdbg
    use HydrologyIsotope, only : hydro_isotope
!!
...
! !ARGUMENTS:
    ...
    integer :: imeg
    real(r8), pointer :: ptr1d(:), ptr2d(:,:)
    integer :: m            ! hydrology tracer index
...
!if(endp>pdbg .and. begp<pdbg) ldbg=.true.
    ...
    do m = 1, pwtrc
       ptr1d => pes%wtr_q_ref2m(:,m)
       call hist_addfld1d (fname='Q2M_'//trim(wtrcnam(m)), units='kg/kg',  &
            avgflag='A', long_name=trim(wtrcnam(m))//'2m specific humidity', &
            ptr_pft=ptr1d)
    end do
    ...
    call hist_addfld1d (fname='RLITTER', units='s/m',  &
         avgflag='M', long_name='litter layer resistance', &
         ptr_pft=pps%rlitter, set_lake=spval, set_urb=spval)

    call hist_addfld1d (fname='RLB', units='s/m',  &
         avgflag='M', long_name='leaf boundary layer resistance', &
         ptr_pft=pps%rlb, set_lake=spval, set_urb=spval)

    call hist_addfld1d (fname='RAW1', units='s/m',  &
         avgflag='M', long_name='canopy-GCM aerodynamic resistance', &
         ptr_pft=pps%raw1, set_lake=spval, set_urb=spval)

    call hist_addfld1d (fname='RAW2', units='s/m',  &
         avgflag='M', long_name='surface-canopy aerodynamic resistance', &
         ptr_pft=pps%raw2, set_lake=spval, set_urb=spval)

    call hist_addfld1d (fname='RSFC', units='s/m',  &
         avgflag='M', long_name='surface boundary layer resistance', &
         ptr_pft=pps%rsfc, set_lake=spval, set_urb=spval)
...
!         default='inactive')
   ...
   !end do
    ...
    ! Tracer hydrology (multiple replicates of all hydrology variables)
    ! (Need to trick fortran into allowing pointer inheretance)

    do m = 1, pwtrc

      ptr1d => pws%RXylem(:,m)
      call hist_addfld1d (fname='RXYLEM_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of xylem water (direct from TracerRatios)', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => cwf%wtr_qflx_runoff(:,m)
      call hist_addfld1d (fname='QRUNOFF_NODYNLNDUSE_'//trim(wtrcnam(m)), units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' total liquid runoff '// &
           '(does not include QSNWCPICE) not including correction for land use change', &
           ptr_col=ptr1d, c2l_scale_type='urbanf')

      ptr1d => cws%wtr_h2osno(:,m)
      call hist_addfld1d (fname='H2OSNO_'//trim(wtrcnam(m)), units='mm',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' snow depth (liquid water)', &
           ptr_col=ptr1d, c2l_scale_type='urbanf')

!#if 0
      ptr1d => pws%wtr_h2ocan(:,m)
      call hist_addfld1d (fname='H2OCAN_'//trim(wtrcnam(m)), units='mm',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' intercepted water', &
           ptr_pft=ptr1d, set_lake=0._r8)
!#endif

      ptr2d => cws%wtr_h2osoi_vol(:,:,m)
      call hist_addfld2d (fname='H2OSOI_'//trim(wtrcnam(m)), units='mm3/mm3', type2d='levgrnd', &
           avgflag='A', long_name=trim(wtrcnam(m))//' volumetric soil water', &
           ptr_col=ptr2d, l2g_scale_type='veg')

      ptr1d => cws%wtr_h2osoi_liqice_10cm(:,m)
      call hist_addfld1d (fname='SOILLIQICE_10CM_'//trim(wtrcnam(m)), units='kg/m2', &
           avgflag='A', long_name=trim(wtrcnam(m))//' soil liquid water and ice in top 10cm', &
           ptr_col=ptr1d)

!! There is a known bug in the writing of SOILLIQ_XXX and SOILICE_XXX to 
!! history files in iCLM4. The j=1 layer actually starts at j=6 (in the hist
!! files), and the first 5 layers are either nonsense or actually the last 5
!! layers. Omit this to avoid confusion until solved. -TW
#if 0
      ptr2d => cws%wtr_h2osoi_liq(:,:,m)
      call hist_addfld2d (fname='SOILLIQ_'//trim(wtrcnam(m)), units='kg/m2', type2d='levgrnd', &
           avgflag='A', long_name=trim(wtrcnam(m))//' soil liquid water', &
           ptr_col=ptr2d, l2g_scale_type='veg')

      ptr2d => cws%wtr_h2osoi_ice(:,:,m)
      call hist_addfld2d (fname='SOILICE_'//trim(wtrcnam(m)), units='kg/m2', type2d='levgrnd', &
           avgflag='A', long_name=trim(wtrcnam(m))//' soil ice', &
           ptr_col=ptr2d, l2g_scale_type='veg')
#endif

      ptr1d => cws%wtr_snowliq(:,m)
      call hist_addfld1d (fname='SNOWLIQ_'//trim(wtrcnam(m)),  units='kg/m2',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' snow liquid water', &
           ptr_col=ptr1d)

      ptr1d => cws%wtr_snowice(:,m)
      call hist_addfld1d (fname='SNOWICE_'//trim(wtrcnam(m)),  units='kg/m2',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' snow ice', &
           ptr_col=ptr1d)

      ptr1d => cwf%wtr_qflx_infl(:,m)
      call hist_addfld1d (fname='QINFL_'//trim(wtrcnam(m)),  units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' infiltration', &
           ptr_col=ptr1d, c2l_scale_type='urbanf')

      ptr1d => cwf%wtr_qflx_surf(:,m)
      call hist_addfld1d (fname='QOVER_'//trim(wtrcnam(m)),  units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' surface runoff', &
           ptr_col=ptr1d, c2l_scale_type='urbanf')

      ptr1d => cwf%wtr_qflx_qrgwl(:,m)
      call hist_addfld1d (fname='QRGWL_'//trim(wtrcnam(m)),  units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' surface runoff at glaciers, wetlands, lakes', &
           ptr_col=ptr1d, c2l_scale_type='urbanf')

      ptr1d => cwf%wtr_qflx_drain(:,m)
      call hist_addfld1d (fname='QDRAI_'//trim(wtrcnam(m)),  units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' sub-surface drainage', &
           ptr_col=ptr1d, c2l_scale_type='urbanf')

      ptr1d => pwf%wtr_qflx_prec_intr(:,m)
      call hist_addfld1d (fname='QINTR_'//trim(wtrcnam(m)), units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' interception', &
           ptr_pft=ptr1d, set_lake=0._r8)

      ptr1d => pwf%wtr_qflx_prec_grnd(:,m)
      call hist_addfld1d (fname='QDRIP_'//trim(wtrcnam(m)), units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' throughfall', &
           ptr_pft=ptr1d, c2l_scale_type='urbanf')

      ptr1d => cwf%wtr_qflx_snomelt(:,m)
      call hist_addfld1d (fname='QMELT_'//trim(wtrcnam(m)),  units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' snow melt', &
           ptr_col=ptr1d, c2l_scale_type='urbanf')

      ptr1d => pwf%wtr_qflx_evap_soi(:,m)
      call hist_addfld1d (fname='QSOIL_'//trim(wtrcnam(m)), units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' ground evaporation', &
           ptr_pft=ptr1d, c2l_scale_type='urbanf')

      ptr1d => pwf%wtr_qflx_evap_can(:,m)
      call hist_addfld1d (fname='QVEGE_'//trim(wtrcnam(m)), units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' canopy evaporation', &
           ptr_pft=ptr1d, set_lake=0._r8, c2l_scale_type='urbanf')

      ptr1d => pwf%wtr_qflx_tran_veg(:,m)
      call hist_addfld1d (fname='QVEGT_'//trim(wtrcnam(m)), units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' canopy transpiration', &
           ptr_pft=ptr1d, set_lake=0._r8, c2l_scale_type='urbanf')

      ptr1d => pws%RCanopyVapor(:,m)
      call hist_addfld1d (fname='RCANV_'//trim(wtrcnam(m)), units='fraction',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of canopy vapor', &
           ptr_pft=ptr1d, set_lake=spval)

!      ptr1d => pws%RCanopyWater(:,m)
!      call hist_addfld1d (fname='RCANW_'//trim(wtrcnam(m)), units='fraction',  &
!           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of canopy water', &
!           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RLeafWaterSunPsn(:,m)
      call hist_addfld1d (fname='RLWSUN_PSN_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit leaf water (incl Peclet), times sunlit photosynthesis', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RLeafWaterShaPsn(:,m)
      call hist_addfld1d (fname='RLWSHA_PSN_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of shaded leaf water (incl Peclet), times shaded photosynthesis', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RLeafWaterSunDiag(:,m)
      call hist_addfld1d (fname='RLWSUN_DIAG_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit leaf water (incl Peclet)', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RLeafWaterShaDiag(:,m)
      call hist_addfld1d (fname='RLWSHA_DIAG_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of shaded leaf water (incl Peclet)', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RLeafWaterSunStdy(:,m)
      call hist_addfld1d (fname='RLWSUN_STDY_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit leaf water (steady state)', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RLeafWaterShaStdy(:,m)
      call hist_addfld1d (fname='RLWSHA_STDY_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of shaded leaf water (steady state)', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RLeafWaterSun(:,m)
      call hist_addfld1d (fname='RLWSUN_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit leaf water', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RLeafWaterSha(:,m)
      call hist_addfld1d (fname='RLWSHA_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of shaded leaf water', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RCelluloseSun(:,m)
      call hist_addfld1d (fname='RCELLSUN_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit cellulose (incl Peclet)', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RCelluloseSha(:,m)
      call hist_addfld1d (fname='RCELLSHA_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of shaded cellulose (incl Peclet)', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RCelluloseSunPsn(:,m)
      call hist_addfld1d (fname='RCELLSUN_PSN_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of sunlit cellulose (incl Peclet), times sunlit photosynthesis', &
           ptr_pft=ptr1d, set_lake=spval)

      ptr1d => pws%RCelluloseShaPsn(:,m)
      call hist_addfld1d (fname='RCELLSHA_PSN_'//trim(wtrcnam(m)), units='fraction', &
           avgflag='A', long_name=trim(wtrcnam(m))//' to H2O ratio of shaded cellulose (incl Peclet), times shaded photosynthesis', &
           ptr_pft=ptr1d, set_lake=spval)

      !Write these using the column-level arrays, otherwise get some weird
      !memory issues
      ptr1d => cws%forc_wtr_q(:,m)
      call hist_addfld1d (fname='QBOT_'//trim(wtrcnam(m)), units='kg/kg',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' atmospheric specific humidity', &
           ptr_col=ptr1d)

      ptr1d => cws%forc_wtr_rain(:,m)
      call hist_addfld1d (fname='RAIN_'//trim(wtrcnam(m)), units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' atmospheric rain', &
           ptr_col=ptr1d)

      ptr1d => cws%forc_wtr_snow(:,m)
      call hist_addfld1d (fname='SNOW_'//trim(wtrcnam(m)), units='mm/s',  &
           avgflag='A', long_name=trim(wtrcnam(m))//' atmospheric snow', &
           ptr_col=ptr1d)
...
!!            ptr_pft=pps%rootr, default='inactive')
       ...
       call hist_addfld2d (fname='ROOTR_COLUMN', units='proportion', type2d='levgrnd', &
            avgflag='A', long_name='effective fraction of roots in each soil layer', &
            ptr_col=cps%rootr_column)   !to get root-wgt iso ratio -TW
!!            ptr_col=cps%rootr_column, default='inactive') !adding root dist to hist output
