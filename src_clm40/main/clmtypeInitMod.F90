...
! !USES:
  ...
  use HydrologyTracer , only : pwtrc
!
! !PUBLIC TYPES:
  implicit none
  save
...
!DEBUG
    use HydrologyTracer, only : pdbg,mdbg,cdbg,pwtrc
!
! !ARGUMENTS:
    implicit none
!
...
!    if(begp<pdbg .and. pdbg<endp) ldbg=.true.
...
!------------------------------------------------------------------------
    ...
    allocate(wbal%wtr_begwb(beg:end,pwtrc))
    allocate(wbal%wtr_endwb(beg:end,pwtrc))
    allocate(wbal%wtr_errh2o(beg:end,pwtrc))

    wbal%begwb(beg:end) = nan
    wbal%endwb(beg:end) = nan
    wbal%errh2o(beg:end) = nan

    wbal%wtr_begwb(beg:end,:) = nan
    wbal%wtr_endwb(beg:end,:) = nan
    wbal%wtr_errh2o(beg:end,:) = nan
...
!------------------------------------------------------------------------
    ...
    allocate(pps%rlitter(beg:end))
    allocate(pps%rlb(beg:end))
    allocate(pps%raw1(beg:end))
    allocate(pps%raw2(beg:end))
    allocate(pps%rsfc(beg:end))
    ...
    allocate(pps%fcaneq(beg:end))
    allocate(pps%fsoieq(beg:end))
    ! 4/14/05: PET
    ! Adding isotope code
    ...
    pps%rlitter(beg:end) = nan
    pps%rlb(beg:end) = nan
    pps%raw1(beg:end) = nan
    pps%raw2(beg:end) = nan
    pps%rsfc(beg:end) = nan
    ...
    pps%fcaneq(beg:end) = 0._r8
    pps%fsoieq(beg:end) = 0._r8
    ! 4/14/05: PET
    ! Adding isotope code
...
!-----------------------------------------------------------------------
    ...
    allocate(pes%wtr_q_ref2m(beg:end,pwtrc))
...
!------------------------------------------------------------------------
    ...
    allocate(pws%wtr_h2ocan(beg:end,pwtrc))
    pws%wtr_h2ocan(beg:end,:) = nan

    allocate(pws%RCanopyVapor(beg:end,pwtrc))
    allocate(pws%RCanopyWater(beg:end,pwtrc))
    allocate(pws%RCelluloseSun(beg:end,pwtrc))
    allocate(pws%RCelluloseSha(beg:end,pwtrc))
    allocate(pws%RCelluloseSunPsn(beg:end,pwtrc))
    allocate(pws%RCelluloseShaPsn(beg:end,pwtrc))
    allocate(pws%RLeafWaterSunDiag(beg:end,pwtrc))
    allocate(pws%RLeafWaterShaDiag(beg:end,pwtrc))
    allocate(pws%RLeafWaterSunStdy(beg:end,pwtrc))
    allocate(pws%RLeafWaterShaStdy(beg:end,pwtrc))
    allocate(pws%RLeafWaterSun(beg:end,pwtrc))
    allocate(pws%RLeafWaterSha(beg:end,pwtrc))
    allocate(pws%RLeafWaterSunPsn(beg:end,pwtrc))
    allocate(pws%RLeafWaterShaPsn(beg:end,pwtrc))
    allocate(pws%RXylem(beg:end,pwtrc))
    pws%RCanopyVapor(beg:end,:) = nan
    pws%RCanopyWater(beg:end,:) = nan
    pws%RCelluloseSun(beg:end,:) = nan
    pws%RCelluloseSha(beg:end,:) = nan
    pws%RCelluloseSunPsn(beg:end,:) = nan
    pws%RCelluloseShaPsn(beg:end,:) = nan
    pws%RLeafWaterSunDiag(beg:end,:) = nan
    pws%RLeafWaterShaDiag(beg:end,:) = nan
    pws%RLeafWaterSunStdy(beg:end,:) = nan
    pws%RLeafWaterShaStdy(beg:end,:) = nan
    pws%RLeafWaterSun(beg:end,:) = nan
    pws%RLeafWaterSha(beg:end,:) = nan
    pws%RLeafWaterSunPsn(beg:end,:) = nan
    pws%RLeafWaterShaPsn(beg:end,:) = nan
    pws%RXylem(beg:end,:) = nan
    allocate(pws%Rdummy(beg:end,pwtrc))
    pws%Rdummy(beg:end,:) = nan
...
!------------------------------------------------------------------------
    ...
    allocate(pwf%wtr_qflx_prec_intr(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_prec_grnd(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_rain_grnd(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_snow_grnd(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_snwcp_liq(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_snwcp_ice(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_tran_veg(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_evap_veg(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_evap_can(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_evap_soi(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_evap_tot(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_evap_grnd(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_dew_grnd(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_sub_snow(beg:end,pwtrc))
    allocate(pwf%wtr_qflx_dew_snow(beg:end,pwtrc))

    pwf%wtr_qflx_prec_intr(beg:end,:) = nan
    pwf%wtr_qflx_prec_grnd(beg:end,:) = spval
    pwf%wtr_qflx_rain_grnd(beg:end,:) = spval
    pwf%wtr_qflx_snow_grnd(beg:end,:) = spval
    pwf%wtr_qflx_snwcp_liq(beg:end,:) = spval
    pwf%wtr_qflx_snwcp_ice(beg:end,:) = spval
    pwf%wtr_qflx_tran_veg(beg:end,:) = spval
    pwf%wtr_qflx_evap_veg(beg:end,:) = nan
    pwf%wtr_qflx_evap_can(beg:end,:) = nan
    pwf%wtr_qflx_evap_soi(beg:end,:) = spval
    pwf%wtr_qflx_evap_tot(beg:end,:) = spval
    pwf%wtr_qflx_evap_grnd(beg:end,:) = spval
    pwf%wtr_qflx_dew_grnd(beg:end,:) = spval
    pwf%wtr_qflx_sub_snow(beg:end,:) = spval
    pwf%wtr_qflx_dew_snow(beg:end,:) = spval
...
!------------------------------------------------------------------------
    ...
    pcf%psnsun(beg:end) = 0._r8
    pcf%psnsha(beg:end) = 0._r8
...
!
!! TODO: should these be defined here or somewhere else?
!!       trying to solve cyclc dependency of
!!       HydrologyTracer (pwtrc) <=> clm_atmlnd (clm_a2l)
!
    allocate(cws%forc_q(beg:end))
    allocate(cws%forc_rain(beg:end))
    allocate(cws%forc_snow(beg:end))
    allocate(cws%forc_wtr_q(beg:end,pwtrc))
    allocate(cws%forc_wtr_rain(beg:end,pwtrc))
    allocate(cws%forc_wtr_snow(beg:end,pwtrc))
    allocate(cws%forc_wtr_Rclim(beg:end,pwtrc))

!    cws%forc_q(beg:end) = 40._r8
!    cws%forc_rain(beg:end) = 40._r8
!    cws%forc_snow(beg:end) = 40._r8
!    cws%forc_wtr_q(beg:end,:) = 40._r8
!    cws%forc_wtr_rain(beg:end,:) = 40._r8
!    cws%forc_wtr_snow(beg:end,:) = 40._r8
    cws%forc_q(beg:end) = nan
    cws%forc_rain(beg:end) = nan
    cws%forc_snow(beg:end) = nan
    cws%forc_wtr_q(beg:end,:) = nan
    cws%forc_wtr_rain(beg:end,:) = nan
    cws%forc_wtr_snow(beg:end,:) = nan
    cws%forc_wtr_Rclim(beg:end,:) = nan
!
    ...
    allocate(cws%wtr_h2osno(beg:end,pwtrc))
    allocate(cws%wtr_h2osoi_liq(beg:end,-nlevsno+1:nlevgrnd,1:pwtrc))
    allocate(cws%wtr_h2osoi_liq_save(beg:end,-nlevsno+1:nlevgrnd,1:pwtrc))
    allocate(cws%wtr_h2osoi_ice(beg:end,-nlevsno+1:nlevgrnd,1:pwtrc))
    allocate(cws%wtr_h2osoi_liqice_10cm(beg:end,pwtrc))
    allocate(cws%wtr_h2osoi_vol(beg:end,1:nlevgrnd,1:pwtrc))
    allocate(cws%wtr_h2osno_old(beg:end,pwtrc))
    allocate(cws%wtr_qg(beg:end,pwtrc))
    allocate(cws%wtr_snowice(beg:end,pwtrc))
    allocate(cws%wtr_snowliq(beg:end,pwtrc))
    allocate(cws%wtr_wa(beg:end,pwtrc))
    allocate(cws%wtr_wt(beg:end,pwtrc))
    allocate(cws%RGNIP(beg:end,pwtrc))

    cws%wtr_h2osno(beg:end,:) = nan
    cws%wtr_h2osoi_liq(beg:end,-nlevsno+1:nlevgrnd,:)= spval
    cws%wtr_h2osoi_liq_save(beg:end,-nlevsno+1:nlevgrnd,:)= spval
    cws%wtr_h2osoi_ice(beg:end,-nlevsno+1:nlevgrnd,:) = spval
    cws%wtr_h2osoi_liqice_10cm(beg:end,:) = spval
    cws%wtr_h2osoi_vol(beg:end,1:nlevgrnd,:) = spval
    cws%wtr_h2osno_old(beg:end,:) = nan
    cws%wtr_qg(beg:end,:) = nan
    cws%wtr_snowice(beg:end,:) = nan
    cws%wtr_snowliq(beg:end,:) = nan
    cws%wtr_wa(beg:end,:) = nan
    cws%wtr_wt(beg:end,:) = nan
    cws%RGNIP(beg:end,:) = nan
...
!------------------------------------------------------------------------
    ...
    allocate(cwf%wtr_qflx_infl(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_surf(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_drain(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_snomelt(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_top_soil(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_sl_top_soil(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_runoff(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_runoff_u(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_runoff_r(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_qrgwl(beg:end,pwtrc))
    allocate(cwf%wtr_qmelt(beg:end,pwtrc))
    allocate(cwf%wtr_h2ocan_loss(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_snofrz_lyr(beg:end,-nlevsno+1:0,pwtrc))
    allocate(cwf%wtr_qflx_snofrz_col(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_irrig(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_glcice(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_glcice_frz(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_glcice_melt(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_rsub_sat(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_floodc(beg:end,pwtrc))
    allocate(cwf%wtr_qflx_snow_melt(beg:end,pwtrc))

    cwf%wtr_qflx_infl(beg:end,:) = nan
    cwf%wtr_qflx_surf(beg:end,:) = nan
    cwf%wtr_qflx_drain(beg:end,:) = nan
    cwf%wtr_qflx_snomelt(beg:end,:) = nan
    cwf%wtr_qflx_top_soil(beg:end,:) = spval
    cwf%wtr_qflx_sl_top_soil(beg:end,:) = nan
    cwf%wtr_qflx_runoff(beg:end,:) = nan
    cwf%wtr_qflx_runoff_u(beg:end,:) = nan
    cwf%wtr_qflx_runoff_r(beg:end,:) = nan
    cwf%wtr_qflx_qrgwl(beg:end,:) = nan
    cwf%wtr_qmelt(beg:end,:) = nan
    cwf%wtr_h2ocan_loss(beg:end,:) = nan
    cwf%wtr_qflx_snofrz_lyr(beg:end,-nlevsno+1:0,:) = spval
    cwf%wtr_qflx_snofrz_col(beg:end,:) = nan
    cwf%wtr_qflx_irrig(beg:end,:)  = nan
    cwf%wtr_qflx_glcice(beg:end,:) = nan
    cwf%wtr_qflx_glcice_frz(beg:end,:) = nan
    cwf%wtr_qflx_glcice_melt(beg:end,:) = nan
    cwf%wtr_qflx_rsub_sat(beg:end,:) = nan
    cwf%wtr_qflx_floodc(beg:end,:) = spval
    cwf%wtr_qflx_snow_melt(beg:end,:) = spval
...
!------------------------------------------------------------------------
    ...
    allocate(gwf%wtr_qflx_runoffg(beg:end,pwtrc))
    allocate(gwf%wtr_qflx_snwcp_iceg(beg:end,pwtrc))
    allocate(gwf%wtr_qflx_liq_dynbal(beg:end,pwtrc))
    allocate(gwf%wtr_qflx_ice_dynbal(beg:end,pwtrc))
    allocate(gwf%wtr_qflx_floodg(beg:end,pwtrc))

    gwf%wtr_qflx_runoffg(beg:end,:) = 0._r8
    gwf%wtr_qflx_snwcp_iceg(beg:end,:) = 0._r8
    gwf%wtr_qflx_liq_dynbal(beg:end,:) = nan
    gwf%wtr_qflx_ice_dynbal(beg:end,:) = nan
    gwf%wtr_qflx_floodg(beg:end,:) = 0._r8 !rtm_flood: initialize to zero for 1st time step instead of nan
...
!------------------------------------------------------------------------
    ...
    allocate(gws%wtr_gc_liq1(beg:end,pwtrc))
    allocate(gws%wtr_gc_liq2(beg:end,pwtrc))
    allocate(gws%wtr_gc_ice1(beg:end,pwtrc))     
    allocate(gws%wtr_gc_ice2(beg:end,pwtrc))    

    gws%wtr_gc_liq1(beg:end,:) = nan
    gws%wtr_gc_liq2(beg:end,:) = nan
    gws%wtr_gc_ice1(beg:end,:) = nan     
    gws%wtr_gc_ice2(beg:end,:) = nan    
