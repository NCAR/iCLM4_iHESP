...
! !USES:
  ...
  use ncdio_pio
  use HydrologyTracer , only : wtrcnam
  implicit none
  save
  private
       ...
       if (present(set_lake)) then 
          do p = begp,endp
             ...
             if (lun%lakpoi(l))  then 
                ! For some reason, this is necessary (deep lakes not being set right...) - TW 29 September 2014
                if(index(fname,'H2OCAN_')==1 .or. index(fname,'RCANV_')==1  .or. &
                   index(fname,'RLWSUN_')==1 .or. index(fname,'RLWSHA_')==1 .or. &
                   index(fname,'RCELLSUN_')==1 .or. index(fname,'RCELLSHA_')==1 .or. &
                   index(fname,'RXYLEM_')==1 .or. &
                   index(fname,'QVEGT_')==1  .or. index(fname,'QVEGE_')==1  .or. index(fname,'QINTR_')==1) then 
                   ptr_pft(p-begp+1) = set_lake
                else 
                   ptr_pft(p) = set_lake
                end if
             end if
