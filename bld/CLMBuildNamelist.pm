...
sub process_namelist_inline_logic {
  ...
  setup_logic_water_isotopes($nl_flags, $definition, $defaults, $nl, $physv);
...
sub setup_logic_initial_conditions {
  ...
  if (not defined $finidat ) {
    ...
    if ( $physv->as_long() == $physv->as_long("clm4_0") ) {
       $settings{'bgc'}    = $nl_flags->{'bgc_mode'};
       foreach my $item ( "mask", "maxpft", "irrig", "glc_nec", "crop", "lnd_tuning_mode", "hydro_tracer" ) {
...
sub setup_logic_water_isotopes {
  #
  # Error checking for water isotope options
  #
  my ($nl_flags, $definition, $defaults, $nl, $physv) = @_;

  if ( $physv->as_long() == $physv->as_long("clm4_0") ) {
     $nl_flags->{'hydro_tracer'} = ".false.";
     my $hyd_trc = $nl->get_value('hydro_tracer');
     if ( defined($hyd_trc) ) {
        $nl_flags->{'hydro_tracer'} = $hyd_trc;
     }
  }
}
